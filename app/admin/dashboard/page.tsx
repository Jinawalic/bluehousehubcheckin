...
export default function AdminDashboardPage() {
  ...
  const stats = useMemo(() => { const total = records.length; return { total, students: records.filter((r) => r.role === 'student').length, staff: records.filter((r) => r.role === 'mentor').length, corpers: records.filter((r) => r.role === 'corper').length, pendingAbsences: absences.filter((a) => a.status === 'pending').length, onTimeRate: total ? Math.round(records.filter((r) => r.status === 'on-time').length / total * 100) : 0 } }, [records, absences])
